...
const Vector3 legZeroOffset(80, 0, -75); // local offset from leg origion to set the zero position somewhere more useful
...
void calcLegServoAngles(Leg_Struct &leg)
{
    ...
    //Vector3 trargrtPosition(0.0f, 0.0f, 120.0f);
    ...
    localCoordinates.x += legZeroOffset.x;
    //Serial.println(localCoordinates.x);                                                  // adjust for zero position (otherwise the leg origion is (0,0,0))
    ...
    //Serial.println(localCoordinates.y);

    localCoordinates.z = leg.targetPosition.z + legZeroOffset.z;
    //Serial.println(localCoordinates.z);
