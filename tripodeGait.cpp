...
void standUp()
{
    if (HexapodState != State::SITTING && HexapodState != State::WALKING)
    {
        return;
    }
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        Leg_update(i); // change this leg_update to output_update
        delay(250);
    }
...
//this sit down method is same as stand up with given coordinates
void sitDown()
{
    ...
    rotation = 0;
    Vector3 startPosition(0.0f, 0.0f, 110.0f);
...
void walkCycle()
{
    ...
    // ######################################
        ...
        //ADD LIMIT SWITCH VALUE INSIDE IF FUNCTION 
...
Vector3 calcTarget(Leg_Struct &leg)
{
    ...
    if (HexapodState == State::STANDING)
    {
        ...
        target.z = LENGTH_TIBIA - (groundClearance + 20);//here i can add the limit switch value =target.z
    ...
    target.z = LENGTH_TIBIA - (groundClearance + 20);
